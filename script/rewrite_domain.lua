...
local function checkStrategyIsExist()
    ...
    ngx.log(ngx.DEBUG, 'reqHost:' .. reqHost)
    -- 根据请求ip查询是否有该策略
    -- redisHelper.Init()
    -- local strategyIp = redisHelper.GetKey(prefix .. reqIp)
    -- redisHelper.Close()
    isExistStrategy = rewriteLogic.isExistHostKey()
    --[[不能用共享变量取策略，因为共享变量会过期]]
    -- local strategyIp = GetShared(reqIp)
    logger.log(httpLogPath, '请求的数据为：%s', 'sourceIp:' .. reqHost)
...
if checkStrategyIsExist() then
    ...
    if isExistStrategy == 1 then
        ...
        -- redisHelper.Close()
        local sValue = rewriteLogic.getHostStrategy()
        if sValue ~= ngx.null then
            local strategy = Json.decode(sValue)
            if strategy ~= nil then
                -- 进行规则过滤
                local isMatch = rewriteLogic.MatchHostRules(strategy)
                if isMatch then
                    -- 符合规则，根据响应类型走不同的流程
                    Log('符合规则')
